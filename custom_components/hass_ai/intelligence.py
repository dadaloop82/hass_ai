...
async def _generate_auto_thresholds(hass: HomeAssistant, entity_id: str, state: State) -> dict:
    ...
    try:
        ...
        if conversation_agent:
            ...
            entity_lower = str(entity_id).lower()  # Ensure it's a string
            
            # EXPLICIT EXCLUSIONS FIRST - these should NEVER be alerts
            explicit_exclusions = [
            ...
            is_explicitly_excluded = any(
                entity_id == excl if not excl.endswith('.') else entity_id.startswith(excl)
                for excl in explicit_exclusions
            )
            
            if is_explicitly_excluded:
                _LOGGER.debug(f"  ❌ EXPLICITLY EXCLUDED: {entity_id}")
                needs_thresholds = False
            else:
                ...
                # 5. Final exclusion for controllable domains (only after value analysis)
                if needs_thresholds:
                    ...
                    controllable_domains = [
                        ...
                        'camera', 'device_tracker'
                    ]
                    
                    # Informational patterns that shouldn't be alerts
                    informational_patterns = [
                        'last_', 'next_', 'current_time', 'uptime', 'version', 
                        'ip_address', 'mac_', '_id', 'attribution', 'integration'
                    ]
